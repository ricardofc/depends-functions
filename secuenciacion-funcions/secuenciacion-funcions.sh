...
if [ $? -eq 0 ]; then
  ...
  NLINHAS=$(cat $1 | wc -l)
  FEXISTS=$(nl -ba -s@ $1 | grep '()' | grep -vE '#|=|"' | awk -F@ '{print $2}' | sed -e 's/^function //g' -e 's/[ \t]*//g' -e 's/\s*//g' | awk -F\( '{print $1}')
  NLINHAANDFEXISTNUMBER=$(nl -ba -s@ $1 | grep '()' | grep -vE '#|=|"' | sed -e 's/[ \t]*//g' -e 's/\s*//g' | awk -F\( '{print $1}' | awk -F@ '{print $1}')
  w=1;for i in $NLINHAANDFEXISTNUMBER; do NLINHAANDFEXIST=$(echo $NLINHAANDFEXIST;echo -ne $i; echo -ne @; echo -ne $FEXISTS|awk -v myvar="$w" '{print $myvar}');w=$(($w+1));done
